...
Combat {
    ...
    function spawnMeatSpray(org, vel) {
        ...
        const meat = meatComponent.createObject(world, { position: org, velocity, avelocity });
        meat.remove.connect(() => { meat?.destroy() })
    }
    ...
    function launch_spike(org, dir, vel, type) {
        ...
        const spike = spikeComponent.createObject(world, settings);
        spike.remove.connect(() => { spike?.destroy() })
        return spike;
    }
    ...
    function launchLaser(org, dir) {
        ...
        const laser = laserComponent.createObject(world, settings);
        laser.remove.connect(() => { laser?.destroy() })
        return laser;
    }
