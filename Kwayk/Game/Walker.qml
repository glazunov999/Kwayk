...
Entity {
    ...
    th.onPhysics: {
        if (effects & Defs.efBrightfield)
            particles.entityParticles(position.plus(center));
    ...
    function throwGib(gibname, dm) {
        ...
        const gib = gibComponent.createObject(world, { position: root.position, name: gibname, velocity, avelocity });
        gib.remove.connect(() => { gib?.destroy() })
    }
    ...
    function dropBackpack() {
        ...
        item.remove.connect(() => { item?.destroy() })
        item.items = weapon;
