...
Walker {
    ...
    Sound {
        id: slimbrn2Sound
        source: "qrc:/Assets/sounds/player/slmbrn2.wav"
    }
    ...
    function fireGrenade() {
        ...
        const grenade = grenadeComponent.createObject(world, settings);
        grenade.remove.connect(() => { grenade?.destroy() })
    ...
    function fireRocket() {
        ...
        const missile = missileComponent.createObject(world, settings);
        missile.remove.connect(() => { missile?.destroy() })
        currentammo = ammo_rockets = ammo_rockets - 1;
    }
    ...
    QtObject {
        ...
        function waterMove() {
            ...
            let wishVel = root.forward.times(forwardMove).plus(root.right.times(sideMove));
